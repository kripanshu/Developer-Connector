...
class ProfileCreds extends Component {
  render() {
    ...
    const expItems = experience.map(exp  =>(
      ...
      <h4>{exp.company}</h4>
      <p> <Moment format="YYYY/MM">{exp.from}</Moment> - {exp.to === null ? ('Now') : <Moment format="YYYY/MM">{exp.from}</Moment>} </p>
    ...
    const eduItems = education.map(edu  =>(
      ...
      <h4>{edu.school}</h4>
      <p> <Moment format="YYYY/MM">{edu.from}</Moment> - {edu.to === null ? ('Now') : <Moment format="YYYY/MM">{edu.from}</Moment>} </p>
